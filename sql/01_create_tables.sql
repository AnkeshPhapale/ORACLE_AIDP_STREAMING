-- Oracle AIDP namespace: aidp_poc.<layer>.<table>
...
-- Current-state master dimensions, sourced from the latest valid interval record.
CREATE TABLE IF NOT EXISTS aidp_poc.silver.device (
  ...
  first_seen_at TIMESTAMP, last_seen_at TIMESTAMP, last_event_id STRING, updated_at TIMESTAMP
) USING DELTA;

CREATE TABLE IF NOT EXISTS aidp_poc.silver.service_point (
  service_point_id STRING, service_point_type STRING, latest_meter_id STRING,
  first_seen_at TIMESTAMP, last_seen_at TIMESTAMP, updated_at TIMESTAMP
) USING DELTA;
