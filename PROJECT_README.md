...
1. In AIDP, upload the repository's `src/` folder and notebooks so `src` is importable by the notebook runtime.
...
4. Run `01_producer.py`, then `02_consumer_bronze.py`, then `03_silver_gold.py`. Verify that rows appear in Bronze, Silver, and Gold before proceeding.
...
7. Run `04_ml_train.py`. Review the printed MAE/RMSE and promote its saved model to the approved path/version configured in `05_ml_score.py` (or update `MODEL_URI` and `MODEL_VERSION` there).
8. Run `05_ml_score.py`, then schedule it after each successful Silver/Gold refresh.
